...
FACILITIES=$INDIR/ipc-facilities.csv
...
cat $FACILITIES \
    ...
    | hxlreplace -p '^$' -s '0' -r -t output+training > $OUTDIR/ipc-coverage-prefecture.csv
...
cat $FACILITIES \
    ...
    | hxlreplace -p '^$' -s '0' -r -t output+training > $OUTDIR/ipc-coverage-sousprefecture.csv
